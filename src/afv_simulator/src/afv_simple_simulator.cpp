#include "afv_simulator/afv_simple_simulator.hpp"

#include "afv_simulator/quaternion_utils.hpp"
...
AfvSimpleSimulator::AfvSimpleSimulator(const rclcpp::NodeOptions & node_options)
: Node("afv_simple_simulator", node_options)
{
  ...
  twist_subscriber_ = create_subscription<geometry_msgs::msg::Twist>(
    "cmd_vel", 10, std::bind(&AfvSimpleSimulator::callback_cmd_vel, this, std::placeholders::_1));
  ...
  timer_ = create_wall_timer(
    ...
    std::bind(&AfvSimpleSimulator::process, this));
}

AfvSimpleSimulator::~AfvSimpleSimulator()
{
}

void AfvSimpleSimulator::callback_cmd_vel(const geometry_msgs::msg::Twist & msg)
{
  cmd_vel_msg_ = msg;
}

void AfvSimpleSimulator::callback_initial_pose(
...
//一定周期ごとにシュミュレート
void AfvSimpleSimulator::process()
{
  current_stamp_ = rclcpp::Clock().now();
...
void AfvSimpleSimulator::publish_tf(
...
RCLCPP_COMPONENTS_REGISTER_NODE(AfvSimpleSimulator)
